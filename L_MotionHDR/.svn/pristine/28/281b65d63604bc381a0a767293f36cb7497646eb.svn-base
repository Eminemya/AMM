...
function outName = phaseAmplify2Y(vidFile, magPhase , fl, fh,fs, outDir, varargin)
    ...
    downVid = zeros(size(res));
    for k = 1:nF
       downVid(:,:,:,k) = imresize(im2single(vid(:,:,:,k)), [size(downVid,1), size(downVid,2)]);
    end
    vid = im2uint8(rgb2y(im2single(downVid)));
    res = im2uint8(rgb2y(im2single(res)));
