...
double OpticalFlow::gradientWeight = 1;
	...
	if(nchannels==1)
	{
		...
		for(int i=0;i<height;i++)
			for(int j=0;j<width;j++)
			{
				...
				data[offset*3+1]=gradientWeight*imdx.data()[offset];
				data[offset*3+2]=gradientWeight*imdy.data()[offset];
			}
	}
	else if(nchannels==3)
	{
		...
		for(int i=0;i<height;i++)
			for(int j=0;j<width;j++)
			{
				...
				data[offset*5+1]=gradientWeight*imdx.data()[offset];
				data[offset*5+2]=gradientWeight*imdy.data()[offset];
