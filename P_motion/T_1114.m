...
DD='data/data_1114/';
sid = 's';
nn = sprintf('50Hz_500fps_%s',sid);im_amp =1; % video frame files
nn = sprintf('100Hz_500fps_%s',sid);im_amp =1; % video frame files
% 
...
if ~exist(vn,'file')
    ...
    for i=1:num_crop
        ...
        pre_mean = mean(tmp_vid(:));
        tmp_vid = U_highpass(tmp_vid,10,f0);
        vid{i} = squeeze(tmp_vid(:,:,1,:))+pre_mean;
        % equalize light
...
ran = {151:295,390:15000};motion_sc =24;mid=3;Gfilt = exp(-(-2:2).^2/2);% 100Hz_500fps_a4_s
...
for i=1:num_crop
    ...
    plot(flow_s{1}(ran{2},1)-flow_s{1}(ran{2}(1),1),flow_s{1}(ran{2},2),'r-')
    set(gca,'FontSize',20)
