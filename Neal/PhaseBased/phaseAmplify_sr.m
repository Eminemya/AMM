...
function outName = phaseAmplify(mid,fr,sr,vidFile, magPhase , fl, fh,fs, outDir, varargin)
    ...
    outName = sprintf('%s-%s-band%0.2f-%0.2f-sr%d-alpha%d-mp%d-sigma%d-scale%0.2f-frames%d-%d-%s-%d-%d-%d.avi', writeTag, func2str(temporalFilter), fl, fh,fs, mean(magPhase(:)), attenuateOtherFreq, sigma, scaleVideo, frames(1), frames(2), repString,mid,nF,sr);
    ...
    magnifiedLumaFFT = zeros(h,w,(nF-1)*sr+1,'single');
    ...
    for level = 2:numLevels-1
        ...
        for frameIDX = 1:nF
            ...
            if frameIDX==nF
                ...
                if mid==0 || (mid==1&&mod(level,8)==2) ||(mid==2&&mod(level,8)==6)
                    % mid=0: all
                    % mid=1: hori
                    % mid=2: verti
                ...
                magnifiedLumaFFT(filtIDX{level,1}, filtIDX{level,2},end) = curLevelFrame + magnifiedLumaFFT(filtIDX{level,1}, filtIDX{level,2},end);
            else                
                for fsr = 1:sr
                  phaseOfFrame = (sr+1-fsr)/sr*delta(:,:,frameIDX)+(fsr-1)/sr*delta(:,:,frameIDX+1);
                    %% Amplitude Weighted Blur        
                    if (sigma~= 0)
                        phaseOfFrame = AmplitudeWeightedBlur(phaseOfFrame, abs(originalLevel)+eps, sigma);        
                    end
                    ...
                    tempTransformOut = exp(1i*phaseOfFrame).*tempOrig; 
                    curLevelFrame = reconLevel(tempTransformOut, level);
                    magnifiedLumaFFT(filtIDX{level,1}, filtIDX{level,2},(frameIDX-1)*sr+fsr) = curLevelFrame + magnifiedLumaFFT(filtIDX{level,1}, filtIDX{level,2},(frameIDX-1)*sr+fsr);
                end
            end
        end
    end
    ...
    for frameIDX = 1:nF
        ...
        if frameIDX ==nF
            magnifiedLumaFFT(filtIDX{level,1},filtIDX{level,2},end) = magnifiedLumaFFT(filtIDX{level,1},filtIDX{level,2},end) + lowpassFrame;    
        else
            for fsr = 1:sr
                magnifiedLumaFFT(filtIDX{level,1},filtIDX{level,2},(frameIDX-1)*sr+fsr) = magnifiedLumaFFT(filtIDX{level,1},filtIDX{level,2},(frameIDX-1)*sr+fsr) + lowpassFrame;    
            end
        end
    end
    ...
    res = zeros(h,w,nC,(nF-1)*sr+1,'uint8');
    ...
    for k = 1:nF
        ...
        if k==nF
            magnifiedLuma = real(ifft2(ifftshift(magnifiedLumaFFT(:,:,end))));
            outFrame(:,:,1) = magnifiedLuma;
            %% Put frame in output
            res(:,:,:,end) = im2uint8(ntsc2rgb(outFrame));
        else
            for fsr =1:sr
                magnifiedLuma = real(ifft2(ifftshift(magnifiedLumaFFT(:,:,(k-1)*sr+fsr))));
                outFrame(:,:,1) = magnifiedLuma;
                %% Put frame in output
                res(:,:,:,(k-1)*sr+fsr) = im2uint8(ntsc2rgb(outFrame));
            end
        end
    end
