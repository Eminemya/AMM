...
function outName = phaseAmplify_trans(mid,fr,region,vidFile, magPhase , fl, fh,fs, outDir, varargin)
    %% Read Video
    ...
    outName = sprintf('%s-%s-band%0.2f-%0.2f-sr%d-alpha%d-mp%d-sigma%d-scale%0.2f-frames%d-%d-%s-%d-%d-%d.avi', writeTag, func2str(temporalFilter), fl, fh,fs, mean(magPhase(:)), attenuateOtherFreq, sigma, scaleVideo, frames(1), frames(2), repString,mid,nF,region(1));
    if redo || ~exist(outName,'file')
        ...
        for level = 2:numLevels-1
            ...
            fprintf('Processing level %d of %d: %d,%d\n', level, numLevels,size(pyrRef,1), size(pyrRef,2));
