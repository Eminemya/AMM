...
function outName = phaseAmplify(mid,fr,sr,vidFile, magPhase , fl, fh,fs, outDir, varargin)
    ...
    outName = sprintf('%s-%s-band%0.2f-%0.2f-sr%d-alpha%d-mp%d-sigma%d-scale%0.2f-frames%d-%d-%s-%d-%d-%d-%d.avi', writeTag, func2str(temporalFilter), fl, fh,fs, mean(magPhase(:)), attenuateOtherFreq, sigma, scaleVideo, frames(1), frames(2), repString,mid,nF,sr(1)+(1+prod(filt_level))*1000,tempF);
